...
#         saveModule = list(model.children())[0]
    ...
    def train(self):
        ...
        train_loader = torch.utils.data.DataLoader(train_set, batch_size = self.batch_size, shuffle=True, num_workers=self.num_workers)

        ## loading validation_loader
        valid_set = RetinalDataset(self.valid_folder, self.size, inference_transformation, phase = 'valid')
        valid_loader = torch.utils.data.DataLoader(valid_set, batch_size = self.batch_size, shuffle=False, num_workers=self.num_workers)
