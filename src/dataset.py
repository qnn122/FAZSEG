...
class FAZ_Preprocess:
    def __init__(self,image, sigma, spacing, tau):
        ...
        image : numpy.ndarray, shape=(H,W)
                A gray-scale OCTA image, value ranging from 0-255
        sigma : float
        spacing : float
        tau: float
        '''
        ...
        if image.dtype == 'float32':
            image = (image*255).astype(np.uint8)                # Make sure pixels fall into [0, 255] range
        if len(image.shape) == 3:
            image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)     # Make sure input image is 2D array
...
class RetinalDataset(utils.Dataset):
    def __init__(self, image_folder, size, image_transform, phase = "valid"):
        ...
        if '.DS_Store' in self.image:
            self.image.remove('.DS_Store')
